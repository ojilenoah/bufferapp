...
export default function BankUser({ route, navigation }) {
  ...
  function onSend() {
    if (!selected) return;
    setSending(true);
    const txnId = "TXN-" + Math.floor(Math.random() * 900000 + 100000).toString();
    const receiptParams = {
    ...
    // Brief buffer then navigate back to Send which shows the overlay
    setTimeout(() => {
    ...
    <View style={styles.safe}>
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={[styles.bankCard, selected?.id === item.id && styles.bankCardActive]}
              onPress={() => setSelected(item)}
            >
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <View style={[styles.bankLogo, { backgroundColor: item.color }]}> 
                  <Text style={styles.bankLogoText}>{item.name.split(' ')[0]}</Text>
                </View>
                <View style={{ marginLeft: 12 }}>
                  <Text style={styles.bankName}>{item.name}</Text>
                  <Text style={styles.bankHint}>{item.hint} •••• {account?.slice(-4) ?? '----'}</Text>
                </View>
              </View>
              {selected?.id === item.id ? (
          ...
          <View style={styles.recipientCard}>
            <View style={styles.recipientLeft}>
              ...
              <View>
                ...
                <Text style={styles.recipientEmail}>{selected.name.replace(' ', '_').toLowerCase()}@mail.com</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <View style={{ marginTop: 20 }}>
          ...
          >
            <Text style={styles.sendBtnText}>{sending ? 'Sending...' : `Send ${country?.symbol ?? '$'}${Number(converted || 0).toFixed(2)}`}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  header: { height: 80, paddingTop: 28, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  iconBtn: { width: 48, alignItems: 'center' },
  backText: { color: '#fff' },
  title: { color: '#fff', fontSize: 18, fontWeight: '700' },
  container: { padding: 16, paddingBottom: 80 },
  summaryWrap: { alignItems: 'center', marginBottom: 12 },
  summaryBadge: { padding: 8, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.03)' },
  summaryIcon: { color: '#13ec80' },
  summaryText: { color: 'rgba(255,255,255,0.7)', marginTop: 8 },
  sectionHeader: { color: 'rgba(255,255,255,0.9)', fontWeight: '700', marginTop: 8 },
  bankCard: { marginTop: 8, padding: 12, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.03)', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  bankCardActive: { borderWidth: 1, borderColor: '#13ec80', backgroundColor: 'rgba(19,236,128,0.05)' },
  bankLogo: { width: 48, height: 48, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  bankLogoText: { color: '#fff', fontWeight: '700' },
  bankName: { color: '#fff', fontWeight: '700' },
  bankHint: { color: 'rgba(255,255,255,0.6)', fontSize: 12 },
  bankCheck: { width: 34, height: 34, borderRadius: 18, backgroundColor: '#13ec80', alignItems: 'center', justifyContent: 'center' },
  recipientCard: { marginTop: 16, backgroundColor: 'rgba(255,255,255,0.03)', borderLeftWidth: 4, borderLeftColor: '#13ec80', padding: 12, borderRadius: 12 },
  recipientLeft: { flexDirection: 'row', alignItems: 'center' },
  recipientAvatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: 'rgba(255,255,255,0.06)' },
  recipientName: { color: '#fff', fontWeight: '800' },
  recipientEmail: { color: 'rgba(255,255,255,0.6)', fontSize: 12 },
  sendBtn: { marginTop: 12, backgroundColor: '#13ec80', paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
...
function resolveRecipientName(bankId, account) {
  // Simulated resolution logic: map last digit to a sample name
            ...
            <Text style={styles.sendBtnText}>
              {sending
                ? "Sending..."
                  ...
                  // Navigate back to the Send screen and show the sending overlay there
                  // The Send screen will present a sending animation then route to TxnSent.
                  setTimeout(() => {
                    navigation.navigate('Send', { sending: true, receiptParams });
                  }, 600);
    flexDirection: "row",
